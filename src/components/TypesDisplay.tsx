...
const TypesDisplay: React.FC = () => {
  ...
  return (
    <div className='grid grid-cols-3 mx-auto gap-7 mt-5'>
      {uniqueTypes.map((type, index) => (
        <div key={index} onClick={()=>{
          ...
          <h2 className='text-zinc-200 p-3 pl-3 text-lg md:text-xl hover:text-zinc-50 overflow-hidden whitespace-nowrap overflow-ellipsis'>{type}</h2>
        </div>
      ))}
    </div>
  );
